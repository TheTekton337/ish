//
//  ApplicationEnvironmentManager.m
//  iSHKit
//
//  Created by Terrance Wood on 3/13/24.
//

#import <iSHKit/ApplicationEnvironmentManager.h>
...
#import <iSHKit/ISHNotifications.h>
...
@implementation ApplicationEnvironmentManager
...
+ (void)setupApplicationEnvironment {
    ...
    // Check and reset "hail mary" setting
    if ([defaults boolForKey:@"hail mary"]) {
        [defaults removeObjectForKey:@"PreferenceBootCommandKey"];
        [defaults removeObjectForKey:@"PreferenceLaunchCommandKey"];
        [defaults setBool:NO forKey:@"hail mary"];
        [defaults synchronize]; // Ensure changes are saved immediately
    }

    // Handle "recovery" mode, could initiate recovery process here if needed
    if (![defaults boolForKey:@"recovery"]) {
        // Example method call, replace with actual boot logic
        // [self boot];
    }
    ...
    [NSNotificationCenter.defaultCenter addObserverForName:UIApplicationWillEnterForegroundNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
        // Example method call, replace with actual sync logic
        // SyncHostname();
    }];
    // SyncHostname(); // Call at initialization if needed
#endif
}
...
static void NetworkReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    [ApplicationEnvironmentManager configureDns];
}

+ (BOOL)handleDidFinishLaunchWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey, id> *)launchOptions {
    // Trigger network permissions popup on Chinese devices
    [[NSURLSession.sharedSession dataTaskWithURL:[NSURL URLWithString:@"http://captive.apple.com"]] resume];

    // Disable animations if running in Fastlane snapshot mode
    if ([NSUserDefaults.standardUserDefaults boolForKey:@"FASTLANE_SNAPSHOT"])
        [UIView setAnimationsEnabled:NO];

#if !ISH_LINUX
    // Set iSH version and hostname override for non-Linux builds
    ...
    // Observe user preferences for disabling screen dimming
    [UserPreferences.shared observe:@[@"shouldDisableDimming"] options:NSKeyValueObservingOptionInitial
                              owner:[UIApplication sharedApplication] usingBlock:^(UIApplication *application) {
        dispatch_async(dispatch_get_main_queue(), ^{
            application.idleTimerDisabled = UserPreferences.shared.shouldDisableDimming;
        });
    }];
    
    // Setup network reachability
    [self configureNetworkReachability];
...
+ (void)configureNetworkReachability {
    ...
    SCNetworkReachabilityContext context = {0, (__bridge void *)([UIApplication sharedApplication]), NULL, NULL, NULL};
...
+ (void)cleanupSharedResources {
    ...
    // - Remove any global observers that were added
    // - Release or nil out any shared resources
    ...
    if (reachabilityRef != NULL) {
        ...
        reachabilityRef = NULL; // Ensure the reference is cleared after release
    }

    // If using NotificationCenter observers
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    
    // If there are other specific cleanup tasks, handle them here
}

// TODO: Research potential issues with App Store review with suspend/exit(0) in iSHKit
- (void)exitApp {
    self.exiting = YES;
